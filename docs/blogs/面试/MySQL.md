...
- （1）对列进行 **计算** 或者是使用函数，则该列的索引会失效
- （2）**不匹配数据类型**，会造成索引失效
- （3）`where`语句中使用了 `IS NULL` 或者 `IS NOT NULL`，会造成索引失效
- （4）使用了 **反向**操作，该索引将不起作用
- （5）使用了 `like`操作，索引就将不起作用
- （6）在 `WHERE` 中使用 `OR`时，**有一个列没有索引**，那么其它列的索引将不起作用